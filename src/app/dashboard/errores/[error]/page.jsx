...
export default function Page({ params }) {
  ...
  const dataError = async () => {
    try {
      const err = await Axios.get(`/api/error/${error}`);
      setErrors(err.data);

      if (err.data.assignedTo === data?.user?.email) {
        setPermission(true);
      }

      const test = await Axios.get(`/api/testcase/${err.data.testCase}`);
      setTests(test.data);
  ...
  return (
    <div className="gap-2 text-white">
      {Permission ? (
          ...
          <div className="pb-2">
            <div className="border border-gray-800 rounded  pb-4 bg-neutral-700">
              <div className="flex justify-between items-center">
                <h2 className="text-white font-bold text-lg p-4">
                  Prueba ID: {test._id}
                </h2>
              </div>
        ...
        <div className="border border-gray-800 rounded  pb-4 bg-neutral-700">
          <div className="flex justify-between items-center">
            <h2 className="text-white font-bold text-lg p-4">Error ID: {errors._id}</h2>
            ...
            >
              <div className="flex justify-between items-center">
                <h2 className=" text-white px-3 py-1 rounded-full text-sm">
                  ...
                  <Link
                    href={`/dashboard/errores/edit/${errors._id}`}
                    className="text-sm bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded"
                  >
                    Editar
                  </Link>
              ...
              <div>
                ...
                <p>{errors.status}</p>
                <p>{errors.assignedTo}</p>
                <p>{errors.reportedby}</p>
                <p>{errors.severity}</p>
                <p>{errors.testCase.title}</p>
                <p>{errors.updatedAt}</p>
